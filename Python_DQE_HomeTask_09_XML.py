## Home task 09: JSON Julia Mendoza Verduzco
...
def process_xml_record(record):  # function to process XML records
    record_type = record.find('type').text  # get the record type
    if record_type == 'News':  # if the record type is News
        text = cleanText(record.find('text').text)  # get the text
        city = cleanText(record.find('city').text)  # get the city
        add_news(text, city)  # add the news
    elif record_type == 'Private Ad':  # if the record type is Private Ad
        text = cleanText(record.find('text').text)  # get the text
        expiration_date = datetime.strptime(record.find('expiration_date').text, '%Y-%m-%d')  # get the expiration date
        add_private_ad(text, expiration_date)  # add the private ad
    elif record_type == 'Book Recommendation':  # if the record type is Book Recommendation
        title = cleanText(record.find('title').text)  # get the title
        author = cleanText(record.find('author').text)  # get the author
        genre = cleanText(record.find('genre').text)  # get the genre
        reason = cleanText(record.find('reason').text)  # get the reason
        add_book_recommendation(title, author, genre, reason)  # add the book recommendation
    else:
        print("Invalid record type in XML file\n")  # print a message that the record type is invalid

def load_records_from_file_xml(file_path):
    if not os.path.exists(file_path):  # check if the file exists
        print("The file does not exist\n")  # print a message that the file does not exist
        return  # return to the main function
    print(f"Loading records from file: {file_path}\n")  # print a message that the records are being loaded from the file
    try:
        ...
        for record in root.findall('record'):  # find all 'record' elements
            process_xml_record(record)  # process each record
        os.remove(file_path)  # remove the file after processing
        print("File processed and removed successfully\n")  # print a message that the file was processed and removed
    except Exception as e:
        print(f"An error occurred processing the XML file: {e}\n")  # print a message that an error occurred
...
def main(): #main function
    while True: #infinite loop to keep adding records
        ...
        if option == '1': #if the option is 1 add news, adds or book recommendations manually
            ...
            file_name = input("Enter the file name (txt, json or xml): ").strip() #request the file name from the user
            ...
            elif os.path.splitext(file_path)[1] == '.xml': #check if the file extension is xml
                load_records_from_file_xml(file_path)  # load the records from the file
            process_statistics()  # call the function to process the statistics
